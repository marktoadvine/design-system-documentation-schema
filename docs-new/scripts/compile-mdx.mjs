 ...
 *   5. Preprocess: replace <ds-prop-table def="…" /> with a placeholder;
 *      render the table from the schema via ./render-prop-table; substitute
 *      the rendered HTML after MDX compiles.
...
// Paths
//
// This file lives in docs/scripts/ — docs/ IS the site root; the repo
// root (where spec/ actually lives) is one level further up.
// ---------------------------------------------------------------------------
...
//
// The schema itself doesn't pin a single version (base.schemaVersion is
// just `type: string` — a document declares which version it targets, not
// the schema). The real single source of truth is the example base
...
// {{VERSION}} token, substituted here at build time. nav.js reads the
// same source, so a single edit to the example's `schemaVersion` propagates
...
function readSpecVersion() {
  ...
  try {
    ...
    const doc = yaml.load(
      fs.readFileSync(
        path.join(EXAMPLES_DIR, "base", "starter-kit.dsds.yaml"),
        "utf-8",
      ),
    );
...
/**
 * Expand `<ds-example file="…" label="…" />` into a fenced YAML code block
 * by reading the corresponding file from `spec/examples/` (ex:
 * `file="entries/button.yaml"` or `file="base/starter-kit.dsds.yaml"`).
 */
...
//
//   <ds-prop-table def="<defName>" />
//
// Examples:
//   <ds-prop-table def="node" />
//   <ds-prop-table def="sections/api" />
//   <ds-prop-table def="common/ref" />
//
// `defName` is a key straight out of dsds.bundled.schema.json's own
// $defs (the same names shown in the nav — bare "entry"/"base"/"section"/
// "metadata", or "common/x" / "sections/x").
//
...
function preprocessPropTables(source, slots) {
  ...
  return source.replace(
    ...
    (_match, attrs) => {
      ...
      if (!defMatch) {
        console.error(`    ⚠  <ds-prop-table> missing required def="…" attribute`);
        return `<!-- ds-prop-table: missing attributes -->`;
      }
      const defName = defMatch[1];

      // Optional `delta` (omit the shared node envelope) and `omit="a,b"`
      // (omit an explicit list) — used by tables that should show only
      // the properties unique to that def.
      const isDelta = /(^|\s)delta(\s|$|=)/.test(attrs);
      const omitMatch = attrs.match(/omit="([^"]+)"/);
      const pathMatch = attrs.match(/path="([^"]+)"/);

      const html = renderPropertyTableForRef(defName, {
      ...
      // (missing def, parse error). Pass those through directly so they
      // remain visible in the output as a diagnostic.
...
/**
 * Compile every .mdx file in docs/content/.
 *

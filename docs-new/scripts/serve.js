 ...
 *
 * Serves the `docs-new/` source tree (not `docs-new/dist/`) so the
 * redesign workbench can load the *real* component ES modules and
 * stylesheets directly — edit a component or token, refresh, see the
 * change. No build step sits between the source and the browser.
 *
 * This is the parallel, DSDS-content-driven site (docs-new/content/*.yaml,
 * validated as real DSDS entries), run alongside the original docs/ site
 * on a different port so both can be compared side by side.
 *
 * Usage:
 *   npm run dev:new              # serves docs-new/ on http://localhost:4301
 *   npm run dev:new -- --port 8080  # custom port
 *   npm run dev:new -- --dist    # serve the built docs-new/dist/ instead
 *
...
const PORT = Number(argValue("--port", process.env.PORT || 4301));
const SERVE_DIST = args.includes("--dist");
const SERVE_DIR = SERVE_DIST ? path.join(ROOT, "dist") : ROOT;
...
server.listen(PORT, () => {
  ...
  const label = SERVE_DIST ? "docs-new/dist" : "docs-new";
  console.log(`\n  DSDS dev server (parallel: docs-new)\n`);

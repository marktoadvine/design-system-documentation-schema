...
- **MUST:** A `check` MUST be present. <small>`common/criterion§verificationMode.1`</small>
...
- **MUST NOT:** When `source` is set, a manifest owns `type`/`values`/`schema`/`defaultValue`/`deprecated` and this document MUST NOT restate them — see `source`. <small>`document-blocks/api§apiProperty.1`</small>
...
- **MUST:** When set, this document is NOT the source of truth for `type`/`values`/`schema`/`defaultValue`/`deprecated` — those MUST be omitted here, and a tool generating or rendering documentation MUST extract them from the manifest at `path` instead. <small>`document-blocks/api§apiProperty.source.1`</small>
...
- **MUST NOT:** When `source` is set, a manifest owns `defaultValue`/`type` and this document MUST NOT restate them — see `source`. <small>`document-blocks/api§apiCssCustomProperty.1`</small>
- **MUST:** MUST be omitted when `source` is set — read from the manifest instead. <small>`document-blocks/api§apiCssCustomProperty.defaultValue.1`</small>
...
- **MUST:** When set, this document is NOT the source of truth for those fields — they MUST be omitted here, and a tool generating or rendering documentation MUST extract them from the manifest at `path` instead. <small>`document-blocks/api§apiMethod.source.1`</small>
...
- **MUST:** MUST be omitted when `token` is set — the token's DTCG file is the source of truth, and this document must not carry a copy that can fork from it. <small>`document-blocks/motion§motionEntry.function.2`</small>
...
- **MUST:** When set, `function` MUST be omitted — the token's DTCG file is the source of truth for the curve. <small>`document-blocks/motion§motionEntry.token.2`</small>
...
- **MUST:** MUST be omitted when `token` is set — the token's DTCG file is the source of truth, and this document must not carry a copy that can fork from it. <small>`document-blocks/scale§scaleStep.value.1`</small>

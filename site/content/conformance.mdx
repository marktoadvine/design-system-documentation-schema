...
*Generated from the v{{VERSION}} schemas by `scripts/extract-normative.mjs` — do not edit by hand. 186 statements: 84 MUST, 28 MUST NOT, 59 SHOULD, 2 SHOULD NOT, 13 MAY.*
...
- **MUST:** Several member types accept a `source` pointing at an external manifest (ex: a Custom Elements Manifest); when set, DSDS is NOT the source of truth for that member's structural facts (type, default value, deprecation, parameters, ...) — those MUST be omitted here, and a tool generating or rendering documentation MUST extract them from the manifest instead. <small>`document-blocks/api§(root).1`</small>
...
- **MUST:** Required unless `source` is set — when a manifest is referenced, `type` MUST be omitted here and read from the manifest instead. <small>`document-blocks/api§apiProperty.type.2`</small>
...
- **MUST:** MUST be omitted when `source` is set — read from the manifest instead. <small>`document-blocks/api§apiProperty.values.2`</small>
...
- **MUST:** MUST be omitted when `source` is set — read from the manifest instead. <small>`document-blocks/api§apiProperty.schema.2`</small>
...
- **MUST:** MUST be omitted when `source` is set — read from the manifest instead. <small>`document-blocks/api§apiProperty.defaultValue.1`</small>
- **MUST:** MUST be omitted when `source` is set — read from the manifest instead. <small>`document-blocks/api§apiProperty.deprecated.1`</small>
...
- **MUST:** MUST be omitted when `source` is set — read from the manifest instead. <small>`document-blocks/api§apiEvent.payload.1`</small>
- **MUST:** When set, this document is NOT the source of truth for `payload` — it MUST be omitted here, and a tool generating or rendering documentation MUST extract it from the manifest at `path` instead. <small>`document-blocks/api§apiEvent.source.1`</small>
...
- **MUST:** MUST be omitted when `source` is set — read from the manifest instead. <small>`document-blocks/api§apiCssCustomProperty.type.1`</small>
- **MUST:** When set, this document is NOT the source of truth for those fields — they MUST be omitted here, and a tool generating or rendering documentation MUST extract them from the manifest at `path` instead. <small>`document-blocks/api§apiCssCustomProperty.source.1`</small>
...
- **MUST:** MUST be omitted when `source` is set — read from the manifest instead. <small>`document-blocks/api§apiMethod.parameters.1`</small>
- **MUST:** MUST be omitted when `source` is set — read from the manifest instead. <small>`document-blocks/api§apiMethod.returnType.1`</small>

...
A tool that checks documents. A conforming validator MUST enforce both the schema itself (with format checks on) and the extra rules below (`DSDS-01`–`DSDS-11`). `scripts/validate/validate.js` is the reference implementation. Its [`examples/invalid/`](examples.html#invalid) folder holds one broken example per semantic rule (`DSDS-XX-*.yaml`) plus a set of plain schema fixtures (`schema-*.yaml`, no rule id — a pure JSON Schema rejection) pinning constraints like enum casing, empty arrays, and `$extensions` namespacing. Every fixture declares its own contract in a leading comment — `# rejectedBy: schema|semantic`, plus `# expect: DSDS-XX` or `# errorAt: /json/pointer` as applicable — and `scripts/validate/conformance-test.js` confirms each one fails for the exact reason and at the exact layer it claims to, not just that it fails at all.

## The words this spec uses

One term per concept, so a reader and a generator mean the same thing by it. The right-hand column is what a reviewer should flag — each of those synonyms was in use somewhere before this table existed.

{/* dsds:terms */}

| Use | For | Not | Defined in |
|---|---|---|---|
| **entry** | One documented thing in the design system graph - a component, token, theme, or anything else. | entity, record, item, node | `entries/entry.schema.yaml` |
| **document** | One `.dsds.yaml` or `.dsds.json` file, whether it holds a whole system or a single entry. | spec, spec file, doc | `base.schema.yaml` |
| **spec** | The DSDS specification itself - this repo, the schema files, and the site that publishes them. | (never a document) |  |
| **section** | One member of an entry's `sections[]`. | block, documentBlock, chunk | `sections/section.schema.yaml` |
...
| **kind** | The discriminator value that says which shape an entry or section is. | type, variant | `entry.kind`, `section.kind` |
| **item** | One member of a section's `items[]`. | entry, rule, criterion |  |
| **conforming consumer** | Anything that reads a document - a renderer, an agent, a site generator. | tool, reader, client, parser, renderer | [Conformance](https://designsystemdocspec.org/conformance#conformance-classes) |
| **conforming producer** | Anything that writes a document. | generator, author tool, writer | [Conformance](https://designsystemdocspec.org/conformance#conformance-classes) |
| **conforming validator** | Anything that checks a document against the schema and the rule catalog. | linter, checker | [Conformance](https://designsystemdocspec.org/conformance#conformance-classes) |
...
| Semantic | By `scripts/validate/validate.js`'s own code (`DSDS-01`–`DSDS-11`: do references resolve, are ids unique, and so on) | Blocks — validation fails |
| Advisory | By `scripts/validate/lint-docs.js` (`npm run lint`, `DSDS-12`+: RFC 2119 casing, a token description that just restates its id, and similar documentation-quality checks) where a check exists — otherwise a judgment call | Never blocks — always exits 0, warnings only |
...
Every catalog entry declares its own tier explicitly, as `enforcement: structural|semantic|advisory|none` — not just implied by which row of this table it's listed in. `scripts/checks/check-rule-catalog.js` (run on every `npm run check`) enforces two things: every entry's `enforcement` is one of those four values, and — for the semantic tier specifically — that the catalog and `scripts/validate/validate.js` agree exactly on which rules exist in both directions. A catalog entry with no matching `RULES.<NAME>` reference in validate.js, or a `RULES.<NAME>` reference with no catalog entry, fails the build instead of silently producing an untraceable `[undefined]` finding.
...
by `scripts/generate/generate-rule-catalog.mjs` — not a hand-kept copy — so it cannot
drift from the machine-readable catalog the validator actually reads.
`scripts/validate/conformance-test.js` and `scripts/checks/check-rule-catalog.js` (both run
on every `npm run check`) additionally confirm the catalog and
`scripts/validate/validate.js`'s own implementation agree. The catalog itself is also
...
<ds-callout variant="warning" title="DSDS-01–DSDS-24 is a reset id space, not a continuation of pre-0.20.0's:">
...
| `DSDS-17` | An entry's own top-level fields don't follow STYLE_GUIDE.md's field order. |
| `DSDS-18` | An entry's `sections[]` doesn't follow STYLE_GUIDE.md's grouping and ordering. |
| `DSDS-19` | A `guidelines` section's items aren't ordered must, should, may, should-not, must-not. |
| `DSDS-20` | A base document's own top-level fields don't follow STYLE_GUIDE.md's field order. |
| `DSDS-21` | A base document's `entries[]` aren't ordered by `kind`. |
| `DSDS-22` | A nested object's fields don't follow the order its own schema file declares. |
| `DSDS-23` | An entry's `combos[]` don't keep one `subject` together, in `level` order. |
| `DSDS-24` | A stable component should identify its accessibility guidance. |

{/* /dsds:rule-catalog */}

`DSDS-06` and `DSDS-07` restore a cycle check the pre-0.20.0 spec had — nothing should point back at itself through a chain of `composes` or `depends-on` links. The 0.20.0 rewrite dropped it by accident; it's enforced by the validator's own code now, not the schema itself.
...
This is exactly what `scripts/validate/conformance-test.js` already does against `scripts/validate/validate.js` on every `npm run check` — the manifest just makes the same check runnable by something that isn't that script.
...
Every RFC 2119 sentence (MUST, MUST NOT, SHOULD, SHOULD NOT, MAY) in the schema's own text — both `description` and `$comment` — extracted and indexed by `scripts/generate/extract-normative.mjs` directly from `schema/**/*.schema.yaml`, not hand-copied, and regenerated on every build (`npm run generate`, checked with `--check` in `generate:check`). Every statement below is in a `description`, where this spec keeps its rules. `$comment` holds the reasoning beside them; a requirement that turns up there is marked *(in `$comment`)* and fails `npm run check`. Each statement's id is location-based (`<dir>/<file>§<jsonPath>.<n>`), so citing one and having the schema move underneath it is a visible, checkable break rather than a silent one.

{/* dsds:normative-index */}

*Generated from the v{{VERSION}} schemas by `scripts/generate/extract-normative.mjs` — do not edit by hand. 16 statements: 12 MUST, 1 MUST NOT, 3 SHOULD, 0 SHOULD NOT, 0 MAY.*

### common

#### common/extensions

- **MUST** — Each top-level key MUST be a dotted namespace, like `com.acme`, matching the Design Tokens Community Group's own `$extensions` convention. <small>`common/extensions§(root).1`</small>

#### common/id

- **MUST** — A section item's own `id` is optional, but a conforming consumer MUST be able to address every item. <small>`common/id§(root).1`</small>
- **MUST** — When an item has no `id`, consumers MUST derive one from the item's own text so content has a consistent id. <small>`common/id§(root).2`</small>

### entries

#### entries/component

- **MUST** — When the document declares a `metadata.platforms` list, this MUST be one of its entries. <small>`entries/component§[allOf][1].sourceFiles.items.platform.1`</small>
- **MUST** — When the document declares a `metadata.platforms` list, this MUST be one of its entries. <small>`entries/component§[allOf][1].imports.items.platform.1`</small>
...
- **SHOULD** — A conforming consumer SHOULD treat `updated.date` as the cache key for this item's documentation specifically. <small>`metadata/metadata§.updated.1`</small>

#### metadata/system-metadata

- **MUST** — When this list is present, every other `platform` value used anywhere in the document MUST match one of these. <small>`metadata/system-metadata§[allOf][1].platforms.1`</small>

### sections

#### sections/definitions

- **MUST** — `id` is optional, but a conforming consumer MUST derive one from this item's own text. <small>`sections/definitions§[allOf][1].items.items.id.1`</small>

#### sections/guidelines

- **SHOULD** — When an item carries a `level` and a ref but no `statement`, a conforming consumer SHOULD show the level and the link rather than rendering it as empty content. <small>`sections/guidelines§(root).1`</small>
- **SHOULD** — An author who expects a person to read the guideline without following the link SHOULD add a `statement` instead. <small>`sections/guidelines§(root).2`</small>
- **MUST** — `id` is optional, but a conforming consumer MUST derive one from this item's own text. <small>`sections/guidelines§[allOf][1].items.items.id.1`</small>
- **MUST** — It MUST reflect `level`. <small>`sections/guidelines§[allOf][1].items.items.example.1`</small>
- **MUST** — A `must-not` item's example MUST show what not to do. <small>`sections/guidelines§[allOf][1].items.items.example.2`</small>

#### sections/section

- **MUST** — `id` is optional, but conforming consumer MUST derive one from this item's own text. <small>`sections/section§freeformEntry.id.1`</small>

#### sections/steps

- **MUST** — `id` is optional, but a conforming consumer MUST derive one from this item's own text. <small>`sections/steps§[allOf][1].items.items.id.1`</small>

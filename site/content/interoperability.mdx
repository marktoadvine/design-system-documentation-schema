...
No open standard covers the entirety of design-system documentation. The standards that exist each cover one layer well. The [W3C Design Tokens format](https://www.w3.org/community/reports/design-tokens/CG-FINAL-format-20251028/) (DTCG) trades token **values**. [Custom Elements Manifest](https://github.com/webcomponents/custom-elements-manifest) (CEM) describes component **code APIs**. DSDS sits at another the layer: it documents meaning, usage, and intent. This schema's philosophy is to **point at** the established formats instead of duplicating them. This page spells out each relationship.

## What DSDS does not define

DSDS is focused on being the source of truth for the documentation itself. Wherever a machine-readable source of truth already exists, DSDS references it and documents around it. 
...
| Component code API facts | CEM manifest (web components) or framework typings | `api` member `source` (`file` + `path`), generatable from the manifest; `links` kind `manifest` for the document-level link |
...
- Token-purpose maps elsewhere in DSDS (anatomy, states, variants) reference tokens using DTCG alias syntax in curly braces (ex: `{color.action.primary}`), and are checked against the documented token layer to make sure they resolve.
...
Every `api` member type carries the same `source` (`file` + `path`) pointer that `token` uses for its DTCG file — mechanically identical, just pointed at a manifest instead. `file` is the manifest's location; `path` locates the specific member within it (ex: `'MyElement.disabled'`), in whatever addressing convention the target file uses. This is what lets a member cite the exact manifest entry it was generated from, the same way a token cites the exact DTCG entry that owns its value.

The intended pipeline for web-component systems: on each release, **generate** the `api` block from the manifest, and mark it `docOrigin.blocks.api = {"origin": "generated", "authorship": "machine-generated"}`. Keep the human-authored layers — guidelines, use cases, accessibility, anatomy — in the same entity. The `docOrigin` field exists so a document can carry both, and a consumer can tell which is which. Link the manifest itself from the entity's `links`, with kind `manifest`, for the document-level relationship; each member's own `source` is the field-level pointer to the exact entry it came from.
        ...
        "properties": [
          {
            ...
            "source": {
              "file": "./my-element.custom-elements.json",
              "path": "MyElement.disabled"
            }
          }
        ],
        "events": [
...
Four things to notice here — they are the interoperability story in miniature.
...
4. `disabled` carries a `source` pointing back at `MyElement.disabled` in the manifest — same shape as a token's `source`, same reason: the manifest stays the fact of record for `type`, and the pointer means that fact can't silently fork from what the DSDS document says.

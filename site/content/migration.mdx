...
slug: migration
description: "How to move DSDS documents between spec versions: field-by-field mappings, the scripts that automate it, and the cases that need a human decision."
---
...
### Scale steps and motion easings can't duplicate a token

If a `scaleStep` or `motionEntry` sets `token`, it can no longer also carry the raw duplicate. Drop the raw field:

| Before | After |
|---|---|
| `{ "token": "space-4", "value": "16px" }` | `{ "token": "space-4" }` |
| `{ "token": "motion-ease-enter", "function": [0.05, 0.7, 0.1, 1] }` | `{ "token": "motion-ease-enter" }` |

A value-only or function-only step (no `token`) is unaffected — this only rejects the combination of both.
...
| Link `identifier`, `role`, `required` | Removed. A link is `kind` + `url` (+ optional `label`). Entities relate through `relationships` entries |

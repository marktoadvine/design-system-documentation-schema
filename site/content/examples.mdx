...
One broken example per semantic rule (`DSDS-XX-*.yaml`) plus plain schema fixtures (`schema-*.yaml`) — the negative-test corpus `scripts/validate/conformance-test.js` runs against.
...
*80 files across 6 categories, generated from the `examples/` directory by `scripts/generate/generate-examples-index.mjs` — do not edit by hand.*

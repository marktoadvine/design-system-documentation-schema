...
A DSDS document has two "readers": people and AI agents. The same file serves both. Most of what you write is for people. A small, optional part is only for agents.

This page explains the split, and when to use each. In short:
...
People read docs to learn a component: what it is, when to use it, how it behaves. Agents read docs to build with it correctly. They look for firm rules, ways to avoid confusion, and how to check their own work.

Most documentation works for both. A clear usage rule helps a person and an agent alike. So you write it once and both readers benefit.
...
- **Telling look-alikes apart:** Notes that keep an agent from mixing this up with a similar entity (ex: "use a link to navigate, not this button").
- **Limits backed by proof:** A rule backed by test evidence. (ex: "agents broke the touch target 9 times out of 10 when they shrank the height — don't")
...
A quick test: would a person reading the docs need this? If yes, it goes in `documentBlocks`. Add to `agentDocumentBlocks` if it'll only help LLMs or tools.

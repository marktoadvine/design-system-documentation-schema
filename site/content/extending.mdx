...
slug: extending
description: "Three ways to go beyond what the spec ships with — $extensions for tool-specific data, custom kinds for a kind of document the spec doesn't have, and profiles for making an existing kind's optional fields required."
---
...
| A custom kind | A kind of document the spec has no name for at all | Nobody, unless you also publish the schema that defines it |
...
`$extensions` on the item itself solves that without loosening the field for everyone:
...
A custom entry kind (`acme.excerpt` above) or a custom section kind (`acme.checklist`, say) is checked against the same **open** base every generic kind is — `entry.schema.yaml` for an entry, `section.schema.yaml` for a section. Open means genuinely open: no schema file exists for `acme.excerpt` in this repo, so nothing stops a misspelled field, or a value of the wrong type, from passing silently. If you want your custom kind to actually enforce anything — required fields, no unexpected properties — you have to write and ship that schema file yourself, in your own copy of `scripts/validate/validate.js`'s schema folder. Neither DSDS itself, nor any other tool, will ever recognize a `kind: acme.excerpt` document as anything more than a set of open fields.

That's the tradeoff with a custom kind: total freedom over the fields, but total unfamiliarity to every other tool. It's the right choice for a document type that's genuinely yours and nobody else's. It's the wrong choice for making a kind the spec already defines stricter — for that, use a profile instead.
...
Save a file at `profiles/entries/<kind>.schema.yaml` or `profiles/sections/<kind>.schema.yaml`, and `scripts/validate/validate.js` picks it up automatically — no code change, nothing to register. `<kind>` can be a well-known kind (`component`, `guidelines`) or a custom one.
...
`profiles/` is never bundled into the published schema. `scripts/generate/bundle.js` only ever looks inside `schema/`, so a project's own profile — which reflects decisions specific to that project, not to the spec itself — can never end up in the published `dsds.bundled.yaml`. Only the command-line validator reads it, and only for the project that added it.
...
- **Separate relationship/link/reference types.** One type already covers it — `common/ref`: `to`/`href` plus `rel`.
...
- **Anything else genuinely absent** — a custom kind, or a profile, depending on whether you're naming a new kind of document or tightening an existing one.

## Choosing between the three

- **Attaching data a specific tool needs, to something the spec already models?** `$extensions`.
- **Documenting something the spec has no place for at all — and you're fine with no other tool understanding it?** A custom kind.
- **Making optional fields mandatory for your own project, without leaving standard DSDS?** A profile.

You can combine them. A custom kind can carry its own `$extensions`. A profile can require that an `$extensions` namespace be present, the same way it requires any other field. Pick the smallest tool that solves your actual problem — reaching for a custom kind just to make one field required gives up far more (the entire fixed structure of the kind you moved away from) than it gains.

...
| `level` | [RequirementLevel](common-requirement-level.md#requirementlevel) | ✓ | Whether the elements in `items` can or cannot be paired with the subject—and how strict the rule is. |
| `note` | string |  | Why this rule holds. |

**References:** `#/$defs/target`, [RequirementLevel](common-requirement-level.md#requirementlevel), [Id](common-id.md#id)

## target {#target}

A bare id, or a token reference wrapped in braces.

**References:** [Id](common-id.md#id)
...
```json
{
  ...
  "properties": {
    ...
    "level": {
      ...
      "$comment": "`must`/`should` make `items` allowed. `should-not`/`must-not` make `items` denied. `must`/`must-not` make the rule explicit. `should`/`should-not` make it a recommendation."
    },
    "note": {
      "type": "string",

...
Everything else on this page (`anatomy`, `api`, `variants`, `states`, `design-specifications`, `motion`, `scale`, and the metadata fields) is **optional**. Use the blocks you need and skip the rest. The [Quick Start](quickstart.html) is the easiest intro. The sections below are the full reference.
...
| `entities/` | Entity schemas (component, token/tokenGroup, theme, foundation, pattern, guide) |
| `document-blocks/` | Block schemas (guidelines, anatomy, api, imports, variants, states, design-specifications, accessibility, scale, principles, motion, content, interactions, sections, steps) Also includes the scoped union (document-blocks.schema.json) |
...
A DSDS file is a JSON object. It needs a `dsdsVersion` and either an `entityGroups` array (for many entities) or an `entity` object (for a single one). Every valid file uses one or the other.
...
Each entry in `entityGroups` is a named group of entities. All of them live in one `entities` array. Their order in the array acts as the intended display order. You can mix kinds freely. Each entity's required `kind` says what type it is.
...
The simplest DSDS document is one file on its own. Splitting a system across files is **optional**. Split files when the system is large enough that pieces are easier to manage.
...
The rules below apply only to tools that resolve `$ref`s. A bundled single file needs none of them. Following a `$ref` can open files or make network requests. Tool that resolve links them must follow a few rules:

- **Relative paths only:** A `$ref` points to a file alongside your document. It can't be a web address or an absolute path. Use [`extends`](#extends) to point at a _different_ design system.
...
- **No reference loops:** If two files reference each other in a circle, the tool treats it as an error.
- **A broken link stops the build:** Tools report a broken link as an error. It does not skip the link or leave a blank.
- **No fetching from the web by default:** This covers every link a tool might follow: `$ref`, `extends`, a token's `source`, and `links`. Turn it on only on purpose. Limit it to sites you trust. Block local and internal addresses (like `169.254.169.254`). That way a hostile file can't make your server reach systems it shouldn't.

These rules keep every tool reading the same split-up document the same way and reduce security risk.
...
The root `systemInfo` object holds the design system's **identity** (name, version, organization, URL, and license). This acts as the system's version of an entity's `identifier` and `name`. It is not the same as the `metadata` object on entities (see [Entity metadata](#entity-metadata)).

<ds-prop-table schema="common/system-info" def="systemInfo" />

### Root document blocks

System-wide docs live in an optional `documentBlocks` array on the root document. They cover the whole system rather than a single artifact. System level blocks work the same as they do on entities, A `use-cases` or `guidelines` block at the root describes the _system_. The same block inside an entity describes that _entity_.
...
Every entity identifies with the same set of properties: a `kind`, an `identifier`, a `name`  (for most kinds), and an optional `description`. It also has a `metadata` object for descriptive fields and a `documentBlocks` array for structured docs. Everything past identity and description (status, tags, and so on) lives as a field in `metadata` (see [Entity metadata](#entity-metadata)). The `documentBlocks` array accepts only the block types that suit that entity type.
...
| `name` | `string` | Varies | Human-readable name. Required on component, foundation, pattern, theme, and guide. Tokens and token-groups use the identifier as its label. |
...
| `relationships` | `array` | No | Typed, directional relationships to other entities — `depends-on`, `composes`, and so on. The structural graph agents read for impact and alternatives. See [Relationships](#relationships). |
| `$extensions` | `object` | No | Vendor-specific extensions. See [Extensions](#extensions). |

### Entity metadata

Descriptive fields past `kind`, `identifier`, `name`, and `description` live in the `metadata` object, keyed by field name. Every field is optional and can appear only once. Keeping them out of the top level keeps entities smaller and more modular. 
...
A nested group of related tokens. The `children` array can hold tokens, nested token groups, or a mix of both. Nesting can be infinite, but don't do that. Accepts **general document block types** only.
...
Each `tokenOverride` names a token through `token` (the token id). It can have a `description` of the change. The actual override values live in the DTCG `source` file.

### Foundation

A broad foundation that governs one subject — color, typography, spacing, elevation, motion, or content. Accepts **foundation-scoped document block types** (principles, scale, motion) and all general document block types.

Required properties: `kind`, `identifier`, `name`. (`description` is an optional top-level property; `status` is an optional `metadata` field — see [Entity metadata](#entity-metadata).)

Foundations document base system topics like color and spacing. They also cover cross-subject concerns like accessibility, motion, and content rules. The `category` property names the domain (ex: `"color"`, `"typography"`, `"spacing"`, `"motion"`, `"accessibility"`, `"content"`).

### Pattern

A broad interaction pattern that defines common, multi-component solutions to a UX problem. Accepts the **pattern-scoped** `interactions` block, the shared structural blocks (anatomy, variants, states), and all general document block types.

Required properties: `kind`, `identifier`, `name`. (`description` is an optional top-level property; `status` is an optional `metadata` field — see [Entity metadata](#entity-metadata).)

A pattern declares the components it uses as typed `composes` entries in its `relationships` array. The `role` field on each entry describes what that component does. There's no dedicated block for this.

### Guide

A long-form, reading-oriented document (ex: a getting-started walkthrough, contribution guide, tutorial, overview, or migration guide). Other entities document one artifact (ex: a component, token, foundation, or pattern). A guide fills in gaps. It documents a *workflow* or a *concept* instead of one thing. Accepts the **guide-scoped** `steps` block, the `imports` block, and all general document block types — including the free-form `sections` block that forms its backbone.
...
A pre-composed block of code that captures a design system pattern. Think of it as a copy-paste starting point built from the system's components. Chunks sit at the same level as components. Where a component documents one building block, a chunk documents a composition of them, captured as code. Chunk stays basic—its `documentBlocks` and `agentDocumentBlocks` accept the general block kinds only.
...
The `code` object carries the source in one of two forms. **Inline** (`code` + `language`) means the source is included in document. **Referenced** (`src` + `language`) means it lives in an external file that the consumer can reference. The referenced `src` is a relative path and follows the same resolution rules as a `$ref` fileRef. Build steps should not inline referenced code. Structured docs live in `documentBlocks`, and agent-only rules live in `agentDocumentBlocks`. Both accept the general block kinds (guidelines, use-cases, accessibility, content, sections, and checklist). A chunk declares the components it uses as typed `composes` in its `relationships` array.
...
Use it for guidance that isn't useful to a human reader. Examples include test-run `evidence`, notes that tell apart similar entities, and machine-aimed examples:
...
The object form adds per-platform readiness. It also adds an optional `note` and a deprecation notice. Deperecation notice becomes required when `overall` is `deprecated`. An entity can be `stable` overall while some platforms are still `experimental` or `draft`.
...
|---|---|---|---|
| `overall` | `statusValue` | Yes | Overall lifecycle status. An editorial call on maturity. It doesn't have to match the highest or lowest platform status. |
...
Every block has a `kind` tag plus its payload. Read one and you can predict the rest:

- **Collection blocks:** Holds a list of like entries in an `items` array: `guidelines`, `sections`, `states`, `variants`, `steps`, `imports`, `interactions`, `motion`, `principles`, `use-cases`. List-shaped blocks have plural names.
...
- **`anatomy` and `scale` use more specific names:** These two collections use a more specific name instead of `items` (`anatomy.parts` and `scale.steps`). These properties are the same idea (a list of entries). The label is more specific for the sake of clarity.

AI/LLM-only content is never added into these blocks. It lives in the entity's separate [`agentDocumentBlocks`](#agent-document-blocks) array, which takes the same block kinds.
...
- **Foundations get subject-level docs:** They govern a domain through principles, value scales, and motion definitions.
- **Tokens only get general block types:** They are single values with usage notes but no visual structure or behavior.
...
One intention exception exists:

- Sub-entry identifiers (on anatomy parts, states, variant values, motion entries, and scale steps) don't follow a pattern. Values like `2xl` are legal here, unlike entity identifiers. This is a deliberate exception.
...
> **Three flavors of guidance:** [`use-cases`](#use-cases-use-cases) says *whether* to use something: the right choice vs. an alternative. `guidelines` says *how* to use it well: concrete rules plus rationale. [`principles`](#principles-principles) (foundations) explains *the system's point of view*.
...
Documents what an entity is for (the scenarios where it is the right choice and where another fits better). These entries help the reader decide _whether_ to use the entity before they use it.
...
The root document uses this same `use-cases` block for system-level adoption guidance (see [Root document blocks](#root-document-blocks)). The full model is in [Use Cases](#use-cases).
...
Use a `guidelines` block with `category: "accessibility"` for **general documentation guidance** (rules with a rationale and a conformance level such as "Provide an aria-label for icon-only buttons"). The accessibility block is not designed for those use cases. The accessibility block records what the artifact *does*. Guidelines say what authors *should do*, and why.
...
Use a `guidelines` block with `category: "content"` for **general documentation guidance** (voice, tone, and writing rules with a rationale and a conformance level, like "Use sentence case"). The content block is not designed for those use cases. A content entry states a reference fact, like "Cancel: closes the current task without saving. Prefer over Close when unsaved work would be lost." A guideline states a rule, like "Use sentence case." 
...
Free-form documentation content in titled, optionally nested sections. It's helpful for overviews, explanations, rationale, and any reading-oriented content the structured blocks miss. It is available on every entity type. Each `sectionEntry` pairs a `title` with an optional rich-text `body`, `examples`, `links`, and nested `sections` for a heading tree. Use `sections` only for content structured blocks can't express.
...
> **Checklist vs. guidelines vs. steps:** [`guidelines`](#guidelines-guidelines) hold the rules and the *why*; `checklist` turns those rules into an explicit pass an agent can tick through; [`steps`](#steps-steps) (guides only) walk a reader through a *procedure* start to finish. Use `checklist` when the goal is an explicit review or integration pass over an existing artifact.
...
Parts represent rendered UI sub-elements (container, label, icon, focus-ring) for components. For patterns, parts represent the structural sections of the pattern's visual layout (image, title, body, primary action).
...
Documents every dimension of visual or behavioral variation. Each item is one axis of config (ex: `"size"`, `"emphasis"`, `"full-width"`). 

Each variant value (and each flag) carries a `tokens` map. These are the token overrides applied when that value is chosen (or the flag is on). Per-variant values live in variants block. `design-specifications` only documents the baseline.

Two optional arrays document how to combine component variants. **`exclusions`** lists combinations that must not be used together. **`requirements`** lists combinations that must not be used together. Each selection points at a variant by `identifier`: a flag on its own, or an enum dimension with a chosen `value`. Every selection MUST resolve to a variant defined in the same block, and each rule carries an RFC 2119 `level` so a tool knows whether it's a hard constraint or a recommendation.
...
```json
{
  ...
  "exclusions": [
    {
      "when": [ { "variant": "icon-only" } ],
      "excludes": [ { "variant": "full-width" } ],
      "level": "must-not",
...
An exclusion needs at least one selection in both `when` and `excludes`. A single value in `excludes` with nothing in `when` wouldn't make sense — if a value is always forbidden, drop it from the dimension's values array instead. Use `must-not` / `should-not` on exclusions and `must` / `should` on requirements.
...
Documents the **default** measurable specs of a component: default design properties, spacing, dimension limits, typography, and responsive behavior. At least one of these sections must be present.

<ds-prop-table schema="document-blocks/design-specifications" def="designSpecifications" />

The base `properties` map is the default spec. Each value is either a token reference or a raw value. The shape says which: a token reference is written as a DTCG alias in braces (`{color.action.primary}`), while a bare value like `16px` or `#0055b3` is a raw literal. When the system has a token layer (declare it with `systemInfo.hasTokenLayer`), reference tokens by alias rather than hardcoding a raw value that could drift. The `tokens` override maps on anatomy/variants/states follow the same rule — values are always token references and so always use braces too.
...
Documents the high-level principles that shape decisions for a subject. They answer "what's our point of view" and "what limits do we accept?" They set direction and sit *above* specific [`guidelines`](#guidelines-guidelines), which hold the concrete rules.
...
The `relationships` array is an entity's structural dependency graph: typed, directional connections to other entities. Where `links` points at external resources, `relationships` records how entities depend on, compose, and supersede one another. This is the graph agents read to reason about impact ("what breaks if this token changes?") and alternatives ("what replaces this?").

Each entry pairs a controlled `relation` with the target entity's `identifier`. `required` marks whether the target is needed for the source to work; `versionConstraint` pins a compatible range; `role` describes what the target does.

<ds-prop-table schema="common/relationship" def="relationship" />

### Relation vocabulary

Write each relationship on the entity that owns it, pointing at the other. A button that depends on a color token goes on the button. A pattern built from a component goes on the pattern. Tools show the reverse automatically — you never write the same relationship twice.
...
- A relationship MUST NOT point at its own entity, and an entity MUST NOT declare the same `(relation, target)` relationship twice.

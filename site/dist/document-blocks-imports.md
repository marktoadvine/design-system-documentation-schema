...
How to install and import a component in code, across platforms. Also reused on guides for setup steps. One entry per platform (ex: React, Web Components, SwiftUI), each with the package name and the import code.
...
| `platform` | string |  | The platform this applies to (ex: 'react', 'web-components', 'swift-ui'). When the system declares `systemInfo.platforms`, this SHOULD match one of those identifiers (the same set the `api` block uses). When left out, it's assumed to be the only/default platform. |
...
```json
{
  ...
  "description": "How to install and import a component in code, across platforms. Also reused on guides for setup steps. One entry per platform (ex: React, Web Components, SwiftUI), each with the package name and the import code.",
  "$defs": {
    "importEntry": {
      "type": "object",
      ...
      "properties": {
        "platform": {
          "type": "string",
          "description": "The platform this applies to (ex: 'react', 'web-components', 'swift-ui'). When the system declares `systemInfo.platforms`, this SHOULD match one of those identifiers (the same set the `api` block uses). When left out, it's assumed to be the only/default platform."
        },
        "package": {
          "type": "string",

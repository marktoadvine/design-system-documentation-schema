...
```json
{
  ...
  "name": "Icon Button",
  "description": "An interactive element that triggers an action when activated. Icon buttons communicate what will happen when the user interacts with them and are the primary mechanism for initiating icon-only actions within a surface.",
  "metadata": {
    "status": {
      "overall": "stable",
      "platforms": {
  ...
  "agentDocumentBlocks": [
    {
      "kind": "use-cases",
      "purpose": "Trigger a user-initiated in-page action using an icon, with no visible text label.",
      "items": [
        {
          "description": "Use icon-button for icon-only, in-page actions; use type-ref for navigation to a URL.",
          "stance": "discouraged",
          "alternative": {
            "identifier": "type-ref"
          }

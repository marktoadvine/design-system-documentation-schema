...
`DSDS-05` and `DSDS-08` both resolve against the whole **project**, not just the one document handed to the validator: a document that declares a `rel: file` ref (the layout the spec itself recommends for a large system) has that link followed transitively, bounded to the repo root, and an unresolved target is re-checked against every entry and shared entry the closure turns up. A target found this way, in a sibling file the validator wasn't directly pointed at, is exactly as resolved as one found locally.

A target genuinely not found anywhere in the project is reported — but only as a **warning**, not a hard failure, and only for the project-scope case. A validator that followed every `rel: file` link it could still can't prove a sibling wasn't missing or unreadable, so it can't assert a pointer is broken with the same confidence it has for a fully self-contained document (where an unresolved target stays a hard error, unchanged). Run with `--strict` to promote these warnings to failures once a project is clean. A `combo`'s trait/token targets aren't resolved yet — that's the next piece of this same proposal.

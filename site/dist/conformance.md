...
A JSON document that validates against the DSDS schema for the version its `dsdsVersion` declares, **and** satisfies the semantic rules that JSON Schema alone can't express. This is the complete list the reference implementation (`scripts/validate.js`) enforces as blocking errors:
...
One reference is resolved against the assembled system rather than a single file, so it is **not** a blocking error here: `useCase.alternative.identifier` (the alternative artifact may live in another file). Tools SHOULD resolve it when the full catalog is available.
